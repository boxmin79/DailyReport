...
def get_thr_img(src, thr, invert=False):
    if invert:
        return cv2.threshold(src, thr, 255, cv2.THRESH_BINARY_INV)[1]
    else:
        return cv2.threshold(src, thr, 255, cv2.THRESH_BINARY)[1]


def get_xor_img(src, thr1, thr2, invert=False):
    img_thr = get_thr_img(src, thr1)
    img_thr_inv = get_thr_img(src, thr2, invert=True)
    result = cv2.bitwise_xor(img_thr, img_thr_inv)
    if invert:
...
def get_contours(src, retr="external"):
    if retr == "external":
        ...
        return cv2.findContours(src, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]
    elif retr == "ccomp":
        return cv2.findContours(src, cv2.RETR_CCOMP, cv2.CHAIN_APPROX_SIMPLE)[0]
